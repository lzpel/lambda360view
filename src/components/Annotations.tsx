...
import { Line, calculateScaleFactor } from '@react-three/drei';
...
function Arrow(props: { start: number[], end: number[], both: boolean, sizeInPx?: number }) {
    const sizeInPx = props.sizeInPx ?? 10;
    // カメラ位置をローカル空間に変換して毎フレーム追跡するで
    const { camera, size } = useThree();
    const groupRef = useRef<THREE.Group>(null);
    const [cameraLocalPos, setCameraLocalPos] = useState(() => camera.position.clone());
    const [arrowSize, setArrowSize] = useState(sizeInPx);
    //計算
    ...
    useFrame(() => {
        if (groupRef.current) {
            ...
            const newArrowSize = calculateScaleFactor(groupRef.current.localToWorld(center.clone()), sizeInPx, camera, size);
            if (Math.abs(newArrowSize - arrowSize) > 0.5) setArrowSize(newArrowSize);
        }
    });
...
const DistanceAnnotationRenderer = ({ annotation }: { annotation: DistanceAnnotation }) => {
    ...
    return (
        <>
            ...
            <Label position={center.toArray() as [number, number, number]} text={annotation.label + (annotation.unit !== undefined ? ` ${distance} ${annotation.unit}` : '')} style={labelStyle} />
        </>
    );
};
